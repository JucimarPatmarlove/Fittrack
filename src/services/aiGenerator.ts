// @ts-nocheck
import { MuscleRecovery } from './fitnessMechanics';
...
export class AIWorkoutGenerator {
    ...
    private static determineAnatolyPhase(history: any[] | undefined, muscleGroup: string): 'powerlifting' | 'bodybuilding' {
        if (!history || history.length === 0) return 'powerlifting';

        const lastWorkout = history
            .filter(w => w && w.exercises && w.exercises.some((e: any) => e && e.muscle === muscleGroup))
            .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];

        if (!lastWorkout) return 'powerlifting';

        const lastExercise = lastWorkout.exercises.find((e: any) => e && e.muscle === muscleGroup);
        const wasPowerlifting = lastExercise && lastExercise.sets && lastExercise.sets.some((s: any) => s && s.reps <= 5);

        return wasPowerlifting ? 'bodybuilding' : 'powerlifting';
    }

    static generateWorkout(recoveredMuscles: MuscleRecovery[], profile: UserProfile, history?: any[]) {
        ...
        const labelPrefix = phase === 'powerlifting' ? '🔥 DIA DE FORÇA (POWERLIFTING)' : '💪 DIA DE VOLUME (BODYBUILDING)';
