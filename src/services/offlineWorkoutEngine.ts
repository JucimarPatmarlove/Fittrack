...
const PREFERENCE_CONFIGS: Record<string, PreferenceConfig> = {
  ...
  peito: {
    ...
    getExercises: (pool) => [
      ...getRandom(pool.peito, 3),
      ...getRandom(pool.ombros, 1),
      ...getRandom(pool.bracos, 1),
    ],
    titleSuffix: 'Peito',
  },
  costas: {
...
function selectExercisePool(profile: Partial<UserProfile>) {
  const isHomeGym =
    profile.availableEquipment?.[0] === 'Apenas Halteres' ||
    profile.availableEquipment?.[0] === 'Peso Corporal';
...
export class OfflineWorkoutEngine {
  static generateSingleWorkout(profile: Partial<UserProfile>, history: WorkoutSession[] = []): any {
    ...
    return {
      ...
      reasoning:
        'Sem ligação à API neural. A usar o motor de fallback local baseado nos teus objetivos principais.',
      exercises,
      exercisesDetails: [],
    };
  }

  static generateWeeklyPlan(profile: Partial<UserProfile>, planType = 'hipertrofia') {
    const pool = selectExercisePool(profile);
    const goal = profile.goal || 'Hipertrofia';
    ...
    return [
      {
        ...
        exercises: [
          ...getRandom(pool.peito, 2),
          ...getRandom(pool.costas, 2),
          ...getRandom(pool.ombros, 1),
        ],
      },
      {
