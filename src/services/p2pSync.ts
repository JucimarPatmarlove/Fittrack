...
class P2PSyncManager {
  ...
  private setupPeerConnection(): void {
    ...
    this.peerConnection.onicecandidate = (event) => {
      if (event.candidate) {
        ...
        window.dispatchEvent(new CustomEvent('p2p-ice-candidate', {
  ...
  private setupDataChannel(): void {
    ...
    this.dataChannel.onopen = () => {
      ...
      this.dataChannel?.send(JSON.stringify({ type: 'ping', from: this.localId, timestamp: Date.now() }));
    };
    ...
    this.dataChannel.onmessage = (event) => {
      try {
        ...
        if (msg.type === 'data') {
          ...
          this.dataChannel?.send(JSON.stringify({ type: 'pong', from: this.localId, timestamp: Date.now() }));
        }
      } catch (err) {
