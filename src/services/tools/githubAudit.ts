    ...
    switch (action) {
      case 'recent_commits': {
        const res = await fetch(
          `${GITHUB_API}/repos/${repo}/commits?sha=${branch}&per_page=5`,
          { headers }
        );
      ...
      case 'file_content': {
        ...
        if (!res.ok) {
          if (res.status === 404) return `Ficheiro "${file_path}" não encontrado na branch "${branch}".`;
      ...
      case 'open_issues': {
        ...
        const issues = await res.json() as Array<{
          number: number; title: string; labels: Array<{ name: string }>;
