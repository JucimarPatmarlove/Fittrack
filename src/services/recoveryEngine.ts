...
function calculateHRVComponent(metrics: RecoveryMetric[]): number {
  ...
  const baseline = sorted.length >= 7
...
function getTrainingAdjustment(
  ...
  if (score >= 85) {
    return { loadModifier: 1.05, volumeModifier: 1.0, maxRPE: 9, suggestedFocus: 'Força máxima ou teste de PR' };
  }
  if (score >= 70) {
    return { loadModifier: 1.0, volumeModifier: 1.0, maxRPE: 8.5, suggestedFocus: 'Treino normal com progressão' };
  }
  if (score >= 55) {
    return { loadModifier: 0.90, volumeModifier: 0.85, maxRPE: 8, suggestedFocus: 'Volume moderado, técnica e controlo' };
  }
  if (score >= 40) {
    return { loadModifier: 0.75, volumeModifier: 0.70, maxRPE: 7, suggestedFocus: 'Recuperação ativa ou mobilidade' };
  }
  return { loadModifier: 0.50, volumeModifier: 0.50, maxRPE: 6, suggestedFocus: 'Descanso completo — prioriza sono e nutrição' };
}
...
export async function logHRV(value: number, source: RecoveryMetric['source'] = 'manual'): Promise<void> {
...
export async function logSleep(hours: number, quality?: number): Promise<void> {
  const { addRecoveryMetric } = await import('../db/schema');
  await addRecoveryMetric({ date: Date.now(), type: 'sleep', value: hours, source: 'manual', notes: quality ? `Qualidade: ${quality}/10` : undefined, timestamp: Date.now() });
}

export async function logSoreness(level: number, muscleGroup?: string): Promise<void> {
  const { addRecoveryMetric } = await import('../db/schema');
  await addRecoveryMetric({ date: Date.now(), type: 'soreness', value: level, source: 'manual', notes: muscleGroup, timestamp: Date.now() });
}

export async function logMood(level: number): Promise<void> {
  const { addRecoveryMetric } = await import('../db/schema');
  await addRecoveryMetric({ date: Date.now(), type: 'mood', value: level, source: 'manual', timestamp: Date.now() });
}
