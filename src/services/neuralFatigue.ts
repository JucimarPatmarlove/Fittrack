...
export async function getEnhancedReadinessScore(
  history: WorkoutSession[],
): Promise<{ score: number; label: string; color: string; clinicalNotes: string[] }> {
  ...
  try {
    ...
    if (metrics) {
      ...
      if (metrics.sleepHours && metrics.sleepHours < 6) {
        penalty += 15;
        clinicalNotes.push(
          `Privação de sono (${metrics.sleepHours.toFixed(1)}h). Risco de lesão elevado e síntese proteica comprometida.`,
        );
