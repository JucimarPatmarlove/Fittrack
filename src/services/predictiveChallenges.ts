...
export const PredictiveChallenges = {
  ...
  predictNextPerformance(history: WorkoutSession[], exerciseName: string): number | null {
    ...
    const dataPoints = workoutsWithEx.map((w, index) => {
      ...
      return { x: index, y: bestWeight === Number.NEGATIVE_INFINITY ? 0 : bestWeight };
    });
  ...
  generateChallenges(history: WorkoutSession[]): Challenge[] {
    ...
    const trendingPool = [
      {
        id: `trend_plank_${Date.now()}`,
        type: 'trending' as const,
        targetExercise: 'Plank Hover',
        targetValue: 60,
        deadline: nextWeek,
        xpReward: 100,
        status: 'active' as const,
        title: 'Plank Hover (Trend 2026)',
        description: 'Aguenta 60s numa variação de Plank Hover, o desafio mais viral de 2026!',
      },
      {
        id: `trend_deadhang_${Date.now()}`,
        type: 'trending' as const,
        targetExercise: 'Dead Hang',
        targetValue: 90,
        deadline: nextWeek,
        xpReward: 150,
        status: 'active' as const,
        title: 'Dead Hang (Trend 2026)',
        description: 'Pendura-te na barra durante 90s. Saúde da coluna e super força de aderência.',
      },
      {
        id: `trend_nord_${Date.now()}`,
        type: 'trending' as const,
        targetExercise: 'Nord Pilates',
        targetValue: 3,
        deadline: nextWeek,
        xpReward: 200,
        status: 'active' as const,
        title: 'Nord Pilates (Trend 2026)',
        description: 'Completa 3 exercícios de estabilidade/Pilates para controlares o corpo.',
      },
    ];
    ...
    if (challenge.type === 'consistency') {
      const weekWorkouts = history.filter(
        (w) =>
          ...
          new Date(challenge.deadline).getTime() - 7 * 24 * 60 * 60 * 1000,
      );
