...
import { ChevronLeft, Play, Square, Zap, Activity, Navigation as NavIcon, Moon, Heart, Triangle, MapPin, Ghost } from 'lucide-react';
...
export default function WalkingCoachScreen({ onClose, onFinish }: Props) {
  ...
  const { state, startTracking, finishTracking, acceptRunningChallenge, dropHazard, speak } = useWalkingCoach(targetPace);
  ...
  // HR color logic
  const hrColor = state.heartRate > 175 ? '#ef4444' : state.heartRate > 150 ? '#f97316' : state.heartRate > 120 ? '#eab308' : '#22c55e';
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#080b0f', color: '#fff', paddingBottom: '96px', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden', fontFamily: '"Outfit", sans-serif' }}>
      ...
      {hrIsAlert && state.isActive && (
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '256px', background: 'rgba(239, 68, 68, 0.15)', filter: 'blur(100px)', pointerEvents: 'none', transition: 'all 0.5s' }} />
      )}
      ...
      <main style={{ flex: 1, padding: '16px', display: 'flex', flexDirection: 'column', alignItems: 'center', zIndex: 10, position: 'relative' }}>
        ...
        <div style={{ width: '100%', maxWidth: '400px', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px', marginBottom: '16px' }}>
        ...
        <div style={{ width: '100%', maxWidth: '400px', display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px', marginBottom: '16px' }}>
          {/* Card Elevação */}
          <div style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '14px', padding: '14px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <span style={{ fontSize: '10px', color: 'rgba(255,255,255,0.4)', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '2px', display: 'flex', alignItems: 'center', gap: '4px' }}>
              <Triangle size={11} color="#eab308" fill="#eab308" /> Elevação
            </span>
            <div style={{ fontSize: '1.6rem', fontWeight: 900, fontFamily: '"DM Mono", monospace', color: '#fff' }}>+{state.elevationGain.toFixed(0)}m</div>
            <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.3)' }}>Alt: {state.altitude.toFixed(0)}m</div>
          </div>
          ...
          <div style={{
            ...
            <div style={{ fontSize: '1.6rem', fontWeight: 900, fontFamily: '"DM Mono", monospace', color: hrColor, transition: 'color 0.3s' }}>
              {state.heartRate.toFixed(0)}
            </div>
            <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.3)' }}>
              {state.heartRate > 175 ? '🔴 ZONA 5' : state.heartRate > 150 ? '🟠 Zona 4' : state.heartRate > 120 ? '🟡 Zona 3' : '🟢 Zona 2'}
            </div>
          </div>
        </div>
        ...
        <div style={{ width: '100%', maxWidth: '400px', display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px', marginBottom: '16px' }}>
          ...
          <div style={{
            ...
            <div style={{ fontSize: '1.5rem', fontWeight: 900, fontFamily: '"DM Mono", monospace', color: '#60a5fa' }}>{formatPace(state.gap)}</div>
            <div style={{ fontSize: '10px', color: '#3b82f6', textTransform: 'uppercase', letterSpacing: '2px', fontWeight: 'bold', marginTop: '4px' }}>G.A.P.</div>
            <div style={{ fontSize: '9px', color: 'rgba(59, 130, 246, 0.5)', marginTop: '2px' }}>Grade Adjusted Pace</div>
          </div>
          ...
          <div style={{
            ...
            <div style={{ fontSize: '1.5rem', fontWeight: 900, fontFamily: '"DM Mono", monospace', color: state.distance > state.ghostDistance ? '#22c55e' : '#94a3b8' }}>
              {state.distance > state.ghostDistance ? '▲' : '▼'} {Math.abs((state.distance - state.ghostDistance) * 1000).toFixed(0)}m
            </div>
            <div style={{ fontSize: '10px', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '2px', fontWeight: 'bold', marginTop: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
        ...
        {/* Passos + Velocidade */}
        <div style={{ width: '100%', maxWidth: '400px', display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px', marginBottom: '24px' }}>
        ...
        <div style={{ width: '100%', maxWidth: '400px', height: '240px', marginBottom: '24px', borderRadius: '16px', boxShadow: '0 4px 15px rgba(0,0,0,0.3)', position: 'relative' }}>
          <WalkingMap path={state.path} isActive={state.isActive} ghostPosition={ghostCoords} hazards={state.hazards} />
        ...
        <div style={{ width: '100%', maxWidth: '400px', marginTop: 'auto' }}>
          ...
          {!state.isActive ? (
            <div style={{ display: 'flex', gap: '12px' }}>
              <button onClick={() => { speak("A iniciar radar avançado."); startTracking(); }} style={{ flex: 2, background: '#1e2532', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', fontWeight: 'bold', padding: '16px', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', boxShadow: '0 4px 15px rgba(0,0,0,0.3)', cursor: 'pointer' }}>
                <Play size={20} fill={C.accent} color={C.accent} /> 
                <span style={{ fontFamily: '"Bebas Neue", sans-serif', fontSize: '1.25rem', letterSpacing: '2px', marginTop: '4px' }}>Iniciar</span>
              </button>
              <button onClick={() => {
                let lat = 40.5373;
                let lng = -7.2658;
                ...
                startTracking();
              }} style={{ flex: 1, background: 'rgba(204,255,0,0.1)', border: `1px solid rgba(204,255,0,0.3)`, color: C.accent, fontWeight: 'bold', padding: '16px', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.875rem', textTransform: 'uppercase', letterSpacing: '1px', cursor: 'pointer' }}>
                Simular
              </button>
            </div>
          ) : (
            <div style={{ display: 'flex', gap: '12px' }}>
              ...
              <button onClick={() => dropHazard('water')} style={{ flex: 1, background: 'rgba(59, 130, 246, 0.1)', border: '1px solid rgba(59, 130, 246, 0.3)', color: '#3b82f6', fontWeight: 'bold', padding: '12px', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px', cursor: 'pointer', fontSize: '11px' }}>
                💧 Água
              </button>
              <button onClick={handleStop} style={{ flex: 2, background: 'rgba(220,38,38,0.2)', border: '1px solid rgba(239,68,68,0.3)', color: '#f87171', fontWeight: 'bold', padding: '16px', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', cursor: 'pointer' }}>
