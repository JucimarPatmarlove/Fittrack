...
export default function Settings({ profile, setProfile, onReset }: SettingsProps) {
  const exportBackup = async () => {
    ...
    const profileData =
      (await get('fit_profile')) || JSON.parse(localStorage.getItem('fit_profile') || '{}');
    const historyData =
      (await get('fit_history')) || JSON.parse(localStorage.getItem('fit_history') || '[]');
    const challengesData =
      (await get('fit_challenges')) || JSON.parse(localStorage.getItem('fit_challenges') || '[]');
  ...
  return (
    <div style={{ padding: '18px', maxWidth: 480, margin: '0 auto' }}>
      <p style={{ fontFamily: "'Bebas Neue'", fontSize: 24, letterSpacing: 2, marginBottom: 18 }}>
        DEFINIÇÕES
      </p>

      <button
        onClick={() =>
          window.dispatchEvent(new CustomEvent('NAVIGATE_TO', { detail: 'milestones' }))
        }
        style={{
          width: '100%',
          ...
          alignItems: 'center',
          boxShadow: `0 4px 15px rgba(232,200,74,0.15)`,
        }}
      >
      ...
      <button
        ...
        style={{
          width: '100%',
          background: `linear-gradient(135deg, rgba(56, 189, 248, 0.1), ${C.bg})`,
          color: '#38bdf8',
      ...
      <div className="glass" style={{ padding: 14, marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
          <div style={{ flex: 1 }}>
            ...
            <input
              ...
              onChange={(e) =>
                setProfile({ ...profile, weight: Number.parseFloat(e.target.value) })
              }
              style={{
                width: '100%',
                background: C.surface,
          ...
          <div style={{ flex: 1 }}>
            ...
            <input
              ...
              onChange={(e) =>
                setProfile({ ...profile, height: Number.parseFloat(e.target.value) })
              }
              style={{
                width: '100%',
                background: C.surface,
        ...
        {profile.gender === 'female' && (
          <label
            style={{
              ...
              gap: 10,
              padding: '10px',
              background: profile.wantsCycleSyncing ? `rgba(167,139,250,0.15)` : C.bg,
              border: `1px solid ${profile.wantsCycleSyncing ? '#a78bfa' : C.border}`,
              borderRadius: 8,
              cursor: 'pointer',
              marginBottom: 16,
            }}
          >
            ...
            <div>
              <span
                style={{
                  fontSize: 14,
                  color: profile.wantsCycleSyncing ? '#a78bfa' : C.text,
                  fontWeight: 'bold',
                }}
              >
                🔄 SINCRONIZAR CICLO MENSTRUAL
              </span>
              <span style={{ display: 'block', fontSize: 11, color: C.muted }}>
                Adapta a intensidade do treino à tua fase do ciclo.
              </span>
            </div>
          </label>
        )}
        ...
        <label
          style={{
            ...
            gap: 10,
            padding: '10px',
            background: profile.proMode ? `${C.accent}22` : C.bg,
            border: `1px solid ${profile.proMode ? C.accent : C.border}`,
            borderRadius: 8,
            cursor: 'pointer',
          }}
        >
          <input
            type="checkbox"
            checked={profile.proMode || false}
            onChange={(e) => setProfile({ ...profile, proMode: e.target.checked })}
          ...
          <div>
            <span
              style={{
                fontSize: 14,
                color: profile.proMode ? C.accent : C.text,
                fontWeight: 'bold',
              }}
            >
              ⚡ ZEN / PRO MODE
            ...
            <span style={{ display: 'block', fontSize: 11, color: C.muted }}>
              Desativa sons, confetis e foca no treino hardcore (RPE).
            </span>
          </div>
        </label>
        ...
        <div style={{ marginTop: 16, paddingTop: 16, borderTop: `1px solid ${C.border}` }}>
          ...
          >
            ⚠️ LESÕES ATIVAS (PROTEÇÃO ARTICULAR)
          </label>
      ...
      <div className="glass" style={{ padding: 14, marginBottom: 20 }}>
        ...
        >
          🧠 MODELO NEURAL (CLAUDE API)
        </p>
        <p style={{ fontSize: 12, color: C.green }}>
          Ligação Segura Cloudflare ativa (Zero-Trust).
        </p>
      </div>
      ...
      >
        <p style={{ fontSize: 11, color: C.muted, lineHeight: 1.5, margin: 0 }}>
          🔒 <strong style={{ color: '#94a3b8' }}>Armazenamento Local:</strong> Todos os teus dados
          (treinos, progresso, definições) são guardados apenas neste navegador. Não sincronizam
          entre dispositivos. Para transferir dados, usa a função de exportação/importação abaixo.
        </p>
      </div>
      ...
      <div className="glass" style={{ padding: 14, marginBottom: 20 }}>
        ...
        <button
          onClick={() => window.dispatchEvent(new CustomEvent('NAVIGATE_TO', { detail: 'backup' }))}
          style={{
            width: '100%',
            background: `linear-gradient(135deg, rgba(232, 200, 74, 0.2), ${C.bg})`,
            color: C.accent,
            border: `1px solid ${C.accent}`,
            borderRadius: 8,
            padding: 12,
        ...
        <p style={{ fontSize: 12, color: C.muted, marginBottom: 8, textAlign: 'center' }}>
          Ou exportar ficheiro local (JSON bruto):
        </p>
