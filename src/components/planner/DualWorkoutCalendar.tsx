...
import { Calendar as CalendarIcon, CheckCircle, Moon, Play, Sun } from 'lucide-react';
  ...
  return (
    <div
      style={{
        marginTop: 16,
        padding: 12,
        background: 'linear-gradient(to right, rgba(232,200,74,0.1), transparent)',
        borderRadius: 8,
      }}
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {workoutsList.map(({ slot, icon: Icon, label, color }) => {
          ...
          if (workout && !rawDayPlan && currentPlan && workout.workoutId === currentPlan.id) {
            ...
            rawDayPlan = currentPlan.workouts.find(
              (w) =>
          ...
          const fallbackPlan =
            rawDayPlan ||
            (workout
              ? { day: dateStr, focus: workout.workoutName, exercises: [workout.workoutName] }
              : null);
          ...
          return (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1 }}>
                ...
                <div>
                  <p style={{ fontSize: 14, fontWeight: 'bold', margin: 0, color: '#fff' }}>
                    {label}
                  </p>
                  {workout ? (
                    <div>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {workout && !workout.completed && onStartWorkout && canStart && !isFuture && (
                  <button
                    onClick={() =>
                      onStartWorkout({
                        id: `day_${workout.id}`,
                        label: `${fallbackPlan!.day}: ${fallbackPlan!.focus}`,
                        exercises: fallbackPlan!.exercises,
                      })
                    }
                    style={{
                      padding: '4px 8px',
                      background: isPast ? '#ef4444' : C.accent,
                ...
                {workout && !workout.completed && !isFuture && (
                  <button
                    ...
                    style={{
                      padding: '4px 12px',
                      background: isPast ? 'rgba(239, 68, 68, 0.2)' : `${C.accent}22`,
                      color: isPast ? '#ef4444' : C.accent,
                      borderRadius: 6,
                      fontSize: 12,
                      border: 'none',
                      cursor: 'pointer',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    {onStartWorkout && canStart && (
                      <button
                        onClick={() =>
                          onStartWorkout({
                            id: `day_${workout.id}_redo`,
                            label: `${fallbackPlan!.day}: ${fallbackPlan!.focus} (Refazer)`,
                            exercises: fallbackPlan!.exercises,
                          })
                        }
                        style={{
                          ...
                          border: `1px solid ${isPast ? '#60a5fa' : C.accent}`,
                          color: isPast ? '#60a5fa' : C.accent,
                          borderRadius: 6,
                          fontSize: 12,
                          cursor: 'pointer',
                          fontWeight: 'bold',
