...
export const SocialModal = ({ onClose }: { onClose: () => void }) => {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(8,11,15,0.9)', backdropFilter: 'blur(10px)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#1a1f25', borderRadius: 16, padding: '20px', width: '90%', maxWidth: 420, maxHeight: '85vh', overflowY: 'auto', border: `1px solid ${C.accent}44` }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
          <h2 style={{ fontFamily: "'Bebas Neue'", fontSize: 28, color: C.accent, margin: 0 }}>REDE SOCIAL</h2>
        ...
        {!isConfigured ? (
          <p style={{ fontSize: 13, color: C.muted, textAlign: 'center', padding: 20 }}>
            A rede social ainda não está configurada neste ambiente
          ...
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              ...
              <button onClick={signOut} style={{ background: 'transparent', border: 'none', color: C.muted, fontSize: 11, cursor: 'pointer', textDecoration: 'underline' }}>
                Terminar sessão
              </button>
            </div>

            <div style={{ display: 'flex', gap: 4, marginBottom: 16, background: '#080b0f', borderRadius: 8, padding: 4 }}>
              {TABS.map((t) => (
                <button
                  key={t.id}
                  onClick={() => setTab(t.id)}
