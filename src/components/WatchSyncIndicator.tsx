...
export function WatchSyncIndicator() {
  ...
  useEffect(() => {
    try {
      const worker = new Worker(new URL('../workers/syncWorker.ts', import.meta.url), { type: 'module' });
  ...
  return (
    <GlassCard style={{ padding: '10px 14px', marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }} onClick={() => window.dispatchEvent(new CustomEvent('NAVIGATE_TO', { detail: 'devices' }))}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <span style={{ fontSize: 22 }}>{mainDev.type === 'hrm' ? '❤️' : '⌚'}</span>
        <div>
          <p style={{ fontSize: 13, fontWeight: 'bold', color: C.text, margin: 0 }}>{mainDev.name}</p>
          <p style={{ fontSize: 10, color: C.muted, margin: 0 }}>{autoSync ? 'Sincronização Ativa' : 'Offline Mode'}</p>
        </div>
      </div>
      {syncPending > 0 ? (
        <div style={{ fontSize: 11, background: `${C.accent}22`, border: `1px solid ${C.accent}`, color: C.accent, padding: '2px 8px', borderRadius: 12, fontWeight: 'bold' }}>
