...
export function FreeWorkoutBuilder({ profile, onClose, onStart }: FreeWorkoutBuilderProps) {
    ...
    if (selectedEquipment.length > 0) {
      pool = pool.filter(ex => {
        return selectedEquipment.some(eq => {
          ...
          if (eq === 'Outros' && ['Kettlebell', 'Bola medicinal', 'Outros'].includes(ex.equipment)) return true;
          return false;
        });
      });
    }
  ...
  useEffect(() => {
    if (step === 3 && exercisesList.length === 0 && fullPool.length > 0) {
      ...
      setExercisesList(shuffled.slice(0, Math.min(workoutDuration, shuffled.length)).map(ex => ex.name));
    }
    ...
    if (newSize > exercisesList.length) {
      ...
      for (let i = 0; i < neededCount; i++) {
        if (shuffledAvailable[i]) {
          ...
          const allDb = Object.keys(EXERCISE_DB).filter(name => !currentNames.has(name) && !newItems.includes(name));
          if (allDb.length > 0) {
  ...
  const CardBtn = ({ item, isSelected, onClick }: any) => (
    <motion.button
      ...
      onClick={onClick}
      style={{
        background: isSelected ? 'linear-gradient(135deg, #e8c84a, #d4b83a)' : 'rgba(18, 25, 35, 0.65)',
    ...
    >
      ...
      <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: '13px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{item.label}</span>
    </motion.button>
  );

  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(8,11,15,0.95)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300, padding: 16, backdropFilter: 'blur(8px)' }}>
      <div style={{ background: C.card, border: `1px solid ${C.accent}`, borderRadius: 16, padding: 20, maxWidth: 440, width: "100%", maxHeight: "90vh", overflowY: "auto", display: "flex", flexDirection: "column", position: "relative", boxShadow: '0 10px 40px rgba(0,0,0,0.6), 0 0 20px rgba(232, 200, 74, 0.15)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: '350px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <button 
                ...
                style={{ background: 'none', border: 'none', color: C.muted, fontSize: '24px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '4px' }}
              >
                ←
              </button>
              <h2 style={{ fontFamily: "'Bebas Neue'", fontSize: '24px', color: C.accent, letterSpacing: '1px', margin: 0 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <button
                  ...
                  style={{
                    background: showAllPool ? 'rgba(232, 200, 74, 0.15)' : 'rgba(255, 255, 255, 0.03)',
            ...
            {/* EXERCISE LIST */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '45vh', overflowY: 'auto', paddingRight: '4px' }}>
              {selectorExercises.length > 0 ? (
                  ...
                  return (
                    ...
                    >
                      <div style={{ minWidth: 0 }}>
                        <p style={{ color: isCurrent ? C.accent : C.text, fontSize: '13px', fontWeight: 600, margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                          {ex.name}
                        </p>
                        <div style={{ display: 'flex', gap: '6px', marginTop: '2px' }}>
                          <span style={{ fontSize: '9px', color: C.muted, display: 'flex', alignItems: 'center', gap: '2px' }}>
                      ...
                      {isCurrent ? (
                        <span style={{ fontSize: '10px', color: C.accent, fontWeight: 'bold', fontFamily: "'DM Mono'" }}>
                          ATUAL
                        </span>
                      ) : isAlreadySelected ? (
                ...
                <div style={{ textAlign: 'center', color: C.muted, padding: '30px 0', fontSize: '13px' }}>
          ...
          <>
            ...
            <AnimatePresence mode="wait">
              ...
              {step === 1 && (
                <motion.div 
                  ...
                  style={{ flex: 1 }}
                >
                  <h2 style={{ fontFamily: "'Bebas Neue'", fontSize: '28px', color: C.accent, letterSpacing: '1px', marginBottom: '4px' }}>O QUE VAMOS TREINAR?</h2>
                  <p style={{ color: C.muted, fontSize: '13px', marginBottom: '20px' }}>Seleciona os grupos musculares que queres trabalhar hoje.</p>
                  
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', maxHeight: '45vh', overflowY: 'auto', paddingRight: '4px' }}>
                    {MUSCLES.map(m => (
                      <CardBtn 
                        key={m.id} 
                        item={m} 
              ...
              {step === 2 && (
                <motion.div 
                  ...
                  style={{ flex: 1 }}
                >
                  <h2 style={{ fontFamily: "'Bebas Neue'", fontSize: '28px', color: C.accent, letterSpacing: '1px', marginBottom: '4px' }}>O QUE TENS DISPONÍVEL?</h2>
                  <p style={{ color: C.muted, fontSize: '13px', marginBottom: '20px' }}>Filtra pelo equipamento que tens à disposição hoje.</p>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', maxHeight: '45vh', overflowY: 'auto', paddingRight: '4px' }}>
                    {EQUIPMENT.map(eq => (
                      <CardBtn 
                        ...
                        onClick={() => toggleSelection(eq.id, selectedEquipment, setSelectedEquipment)} 
                      />
                    ))}
                  </div>
                </motion.div>
              ...
              {step === 3 && (
                ...
                >
                  <div style={{ textAlign: 'center', marginBottom: '16px' }}>
                    <span style={{ fontSize: '36px', display: 'block', marginBottom: '4px', filter: 'drop-shadow(0 0 10px rgba(232,200,74,0.4))' }}>⚡</span>
                    <h2 style={{ fontFamily: "'Bebas Neue'", fontSize: '28px', color: C.text, letterSpacing: '1px', marginBottom: '2px' }}>CONFIGURAÇÃO DO TREINO</h2>
                    <p style={{ color: exercisesList.length > 0 ? C.green : C.red, fontSize: '12px', fontWeight: 'bold' }}>
                  ...
                  {exercisesList.length > 0 ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                      ...
                      <div style={{ background: 'rgba(255,255,255,0.02)', border: `1px solid ${C.border}`, borderRadius: '12px', padding: '12px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <span style={{ fontSize: '11px', color: C.muted, fontWeight: 'bold', fontFamily: "'DM Mono'" }}>QUANTIDADE:</span>
                          <div style={{ display: 'flex', gap: '6px' }}>
                            {[3, 4, 5, 6, 7].map(num => (
                              <button 
                                ...
                                style={{
                                  background: workoutDuration === num ? C.accent : 'rgba(0,0,0,0.3)',
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <span style={{ fontSize: '11px', color: C.muted, fontWeight: 'bold', fontFamily: "'DM Mono'" }}>FORMATO:</span>
                          <div style={{ display: 'flex', gap: '6px' }}>
                            <button 
                              onClick={() => setWorkoutFormat('standard')}
                              style={{
                                background: workoutFormat === 'standard' ? C.accent : 'rgba(0,0,0,0.3)',
                            ...
                            <button 
                              ...
                              style={{
                                background: workoutFormat === 'circuit' ? C.accent : 'rgba(0,0,0,0.3)',
                        ...
                        {workoutFormat === 'circuit' && (
                          <motion.div 
                            ...
                            style={{ borderTop: `1px solid ${C.border}`, paddingTop: '10px', display: 'flex', flexDirection: 'column', gap: '8px' }}
                          >
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                              <span style={{ fontSize: '11px', color: C.muted, fontFamily: "'DM Mono'" }}>Rondas:</span>
                              <div style={{ display: 'flex', gap: '6px' }}>
                                {[2, 3, 4, 5].map(r => (
                                  <button 
                                    key={r}
                                    ...
                                    style={{
                                      background: circuitRounds === r ? C.accent : 'rgba(0,0,0,0.3)',
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                              <span style={{ fontSize: '11px', color: C.muted, fontFamily: "'DM Mono'" }}>Descanso entre rondas:</span>
                              <div style={{ display: 'flex', gap: '6px' }}>
                                {[60, 90, 120, 180].map(sec => (
                                  <button 
                                    ...
                                    style={{
                                      background: circuitRest === sec ? C.accent : 'rgba(0,0,0,0.3)',
                      ...
                      <div style={{ background: 'rgba(14, 19, 24, 0.8)', borderRadius: '12px', padding: '12px', border: `1px solid ${C.border}`, display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '28vh', overflowY: 'auto' }}>
                        {exercisesList.map((name, idx) => {
                          ...
                          return (
                            <div key={idx} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: idx < exercisesList.length - 1 ? `1px solid ${C.border}44` : 'none', paddingBottom: idx < exercisesList.length - 1 ? '8px' : '0' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', minWidth: 0 }}>
                                <div style={{ width: '22px', height: '22px', borderRadius: '50%', background: C.border, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontFamily: "'DM Mono'", color: C.accent }}>
                                  {idx + 1}
                                </div>
                                <div style={{ minWidth: 0 }}>
                                  <p style={{ color: C.text, fontSize: '13px', fontWeight: 600, margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{name}</p>
                                  <div style={{ display: 'flex', gap: '6px', marginTop: '2px' }}>
                                    <span style={{ fontSize: '9px', background: 'rgba(255,255,255,0.05)', color: C.muted, padding: '1px 4px', borderRadius: '4px', display: 'flex', alignItems: 'center', gap: '3px' }}>
                    ...
                    <div style={{ background: 'rgba(232, 74, 74, 0.05)', borderRadius: '12px', padding: '20px', border: `1px dashed ${C.red}`, textAlign: 'center', color: C.muted, fontSize: '13px' }}>
                      A combinação selecionada é muito restrita. Volta atrás e seleciona mais grupos musculares ou tipos de equipamento.
                    </div>
                  )}
                </motion.div>
              )}
            ...
            <button 
              ...
              style={{ 
                marginTop: '20px', 
                width: '100%', 
                background: ((step === 1 && selectedMuscles.length === 0) || (step === 2 && selectedEquipment.length === 0) || (step === 3 && exercisesList.length === 0)) ? C.border : 'linear-gradient(135deg, #e8c84a, #d4b83a)', 
                color: ((step === 1 && selectedMuscles.length === 0) || (step === 2 && selectedEquipment.length === 0) || (step === 3 && exercisesList.length === 0)) ? C.muted : '#000', 
                border: 'none', 
                borderRadius: '12px', 
                padding: '16px', 
                fontFamily: "'Bebas Neue'", 
                fontSize: '18px', 
                letterSpacing: '1px', 
                cursor: ((step === 1 && selectedMuscles.length === 0) || (step === 2 && selectedEquipment.length === 0) || (step === 3 && exercisesList.length === 0)) ? 'not-allowed' : 'pointer', 
                transition: 'background 0.3s, color 0.3s, transform 0.1s',
                boxShadow: ((step === 1 && selectedMuscles.length === 0) || (step === 2 && selectedEquipment.length === 0) || (step === 3 && exercisesList.length === 0)) ? 'none' : '0 4px 15px rgba(232, 200, 74, 0.3)'
              }}
            >
