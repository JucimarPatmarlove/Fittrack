...
const MUSCLE_COLOR: Record<string, string> = {
  Peito: '#e84a4a', Dors: '#3dd68c', Trapézio: '#3dd68c',
  Quadríceps: '#e8c84a', Glúteos: '#f472b6', Isquiotibiais: '#fb923c',
  Ombros: '#38bdf8', Bíceps: '#60a5fa', Tríceps: '#a78bfa',
  ABS: '#fb923c', Oblíquos: '#fb923c',
...
const ExerciseThumb = ({ name, emoji }: { name: string; emoji: string }) => {
  ...
  return (
    <div style={{
      ...
      overflow: 'hidden', borderRadius: 8, background: '#080b0f',
    }}>
      {src ? (
        <img
          src={src}
  ...
  return (
    <AnimatePresence>
      <div style={modalOverlay} onClick={e => { if (e.target === e.currentTarget) { setOpenFilter(null); onClose(); } }}>
        ...
        >
          {/* Drag handle */}
          <div style={{ width: 40, height: 5, background: '#1e2832', borderRadius: 3, margin: '12px auto 0' }} />

          {/* Header */}
          <div style={headerRow}>
            <button onClick={onClose} style={headerBtn(C.blue)}>Fechar</button>
            <span style={{ fontWeight: 700, fontSize: 18, color: C.text, fontFamily: "'Outfit', sans-serif" }}>
              Biblioteca
            </span>
            {/* Grid / List toggle */}
            <div style={{ display: 'flex', gap: 4 }}>
              {(['grid', 'list'] as ViewMode[]).map(v => (
                <button
                  ...
                  onClick={() => setView(v)}
                  style={{
                    padding: '4px 10px', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 14,
          ...
          <div style={searchWrap}>
            ...
            />
            {search && (
              <button onClick={() => setSearch('')} style={{ background: 'none', border: 'none', color: C.muted, fontSize: 16, cursor: 'pointer' }}>✕</button>
            )}
          </div>
          ...
          <div style={filterRow} onClick={() => setOpenFilter(null)}>
            {[
              { key: 'muscle' as const, label: muscle !== 'Todos' ? muscle : 'Músculos', active: muscle !== 'Todos' },
              { key: 'equipment' as const, label: equipment !== 'Todos os equipamentos' ? equipment : 'Equipamento', active: equipment !== 'Todos os equipamentos' },
              { key: 'type' as const, label: actType !== 'Todos os tipos' ? actType : 'Tipo', active: actType !== 'Todos os tipos' },
            ].map(f => (
              <button
                key={f.key}
          ...
          <AnimatePresence>
            {activeFilters.length > 0 && (
              <motion.div
                ...
                style={{ padding: '4px 16px', display: 'flex', gap: 8, flexWrap: 'wrap', overflow: 'hidden' }}
              >
                {activeFilters.map(f => (
                  <span key={f} style={activeBadge}>
                    ...
                    <button
                      ...
                      style={{ background: 'none', border: 'none', color: C.accent, marginLeft: 6, fontSize: 12, cursor: 'pointer' }}
                    >✕</button>
                  </span>
                ))}
              </motion.div>
              ...
              >
                <div style={dropdown} onClick={e => e.stopPropagation()}>
                  {openFilter === 'muscle' && MUSCLE_GROUPS.map(g => (
                    <button key={g} onClick={() => { setMuscle(g); setOpenFilter(null); }} style={dropdownItem(muscle === g)}>{g}</button>
                  ))}
                  {openFilter === 'equipment' && EQUIPMENT_TYPES.map(g => (
                    <button key={g} onClick={() => { setEquipment(g); setOpenFilter(null); }} style={dropdownItem(equipment === g)}>{g}</button>
                  ))}
                  {openFilter === 'type' && ACTIVITY_TYPES.map(g => (
                    <button key={g} onClick={() => { setActType(g); setOpenFilter(null); }} style={dropdownItem(actType === g)}>{g}</button>
                  ))}
                </div>
              </motion.div>
            )}
          ...
          {/* Results count */}
          <div style={{ padding: '6px 20px 2px', fontSize: 11, color: C.muted, fontFamily: 'monospace' }}>
            {filtered.length} exercício{filtered.length !== 1 ? 's' : ''}
            {selected.length > 0 && (
              <span style={{ color: C.accent, marginLeft: 8 }}>· {selected.length} seleccionado{selected.length !== 1 ? 's' : ''}</span>
            )}
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div style={{ padding: '8px 10px 10px' }}>
                        <p style={{
                          fontSize: 12, fontWeight: 600, color: isSel ? color : C.text,
                        ...
                        <p style={{
                          fontSize: 10, color: C.muted, margin: '4px 0 0',
                  ...
                  return (
                    ...
                    >
                        ...
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                          {ex.muscles.slice(0, 2).map((m, i) => (
                            <span key={i} style={{
                              ...
                              color: muscleColor(m),
                              fontWeight: 600, border: `1px solid ${muscleColor(m)}33`,
                            }}>
                              {m}
                            </span>
                          ))}
                      ...
                      <div style={{
                        ...
                        {isSel && (
                          <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} style={{ color: '#000', fontSize: 13, fontWeight: 800 }}>
                            ✓
                          </motion.span>
                        )}
                      </div>
                ...
                <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                  <button
                    ...
                    style={{
                      ...
                      color: C.muted, padding: '14px 16px', borderRadius: 12, cursor: 'pointer', fontSize: 13,
                    }}
                  >
                    Limpar
                  </button>
...
const searchWrap: React.CSSProperties = {
  margin: '6px 16px', background: C.card, borderRadius: 14,
  border: `1px solid ${C.border}`, padding: '11px 16px',
...
const dropdown: React.CSSProperties = {
  ...
  boxShadow: '0 16px 48px rgba(0,0,0,0.9)', zIndex: 51,
};
const dropdownItem = (active: boolean): React.CSSProperties => ({
  ...
  fontSize: 15, fontWeight: active ? 600 : 400, cursor: 'pointer',
});
