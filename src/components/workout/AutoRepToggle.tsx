...
export const AutoRepToggle: React.FC<AutoRepToggleProps> = ({ onRepDetected, isActive, onToggle }) => {
  ...
  return (
    ...
    >
      <div className={`relative flex items-center justify-center w-10 h-10 rounded-full ${isActive ? 'bg-cyan-500' : 'bg-gray-600'}`}>
      ...
      <div className="ml-auto">
        <div className={`w-10 h-5 rounded-full p-1 flex items-center transition-colors ${isActive ? 'bg-cyan-500' : 'bg-gray-600'}`}>
          <div className={`w-3 h-3 bg-white rounded-full shadow-md transform transition-transform ${isActive ? 'translate-x-5' : 'translate-x-0'}`} />
        </div>
      </div>
    </div>
  );
