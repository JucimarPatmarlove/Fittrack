...
export function WeeklyPlanGenerator({ profile, setProfile, onStartWorkout, onClose }: WeeklyPlanGeneratorProps) {
    ...
    const [trainingDays, setTrainingDays] = useState<string[]>(profile.trainingDays || ["Segunda", "Quarta", "Sexta"]);
    ...
    return (
        <div style={{ position: "fixed", inset: 0, background: "rgba(8,11,15,0.95)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300, padding: 24, backdropFilter: 'blur(5px)' }}>
            <div style={{ background: C.card, border: `1px solid ${C.accent}`, borderRadius: 14, padding: 26, maxWidth: 400, width: "100%", maxHeight: "85vh", overflowY: "auto" }}>
                
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                    <p style={{ fontFamily: "'Bebas Neue'", fontSize: 24, letterSpacing: 2, color: C.accent, margin: 0 }}>📅 PLANO DE ELITE IA</p>
                    <button onClick={onClose} style={{ background: 'none', border: 'none', color: C.muted, fontSize: 20, cursor: 'pointer' }}>×</button>
                </div>

                {step === 'info' && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                        <p style={{ fontSize: 13, color: '#fff', marginBottom: 16 }}>Configura as variáveis de biofeedback para uma precisão de Personal Trainer.</p>
                        
                        <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
                            <div style={{ flex: 1 }}>
                                <label style={{ display: "block", fontSize: 12, color: C.muted, marginBottom: 4 }}>GÉNERO</label>
                                <select value={gender} onChange={e => setGender(e.target.value)} style={{ width: "100%", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: 10, color: C.text }}>
                            ...
                            <div style={{ flex: 1 }}>
                                ...
                                <input type="number" value={age} onChange={e => setAge(e.target.value)} style={{ width: "100%", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: 10, color: C.text }} />
                            </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
                            <div style={{ flex: 1 }}>
                                ...
                                <input type="number" value={weight} onChange={e => setWeight(e.target.value)} style={{ width: "100%", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: 10, color: C.text }} />
                            </div>
                            <div style={{ flex: 1 }}>
                                <label style={{ display: "block", fontSize: 12, color: C.muted, marginBottom: 4 }}>ALTURA (CM)</label>
                                <input type="number" value={height} onChange={e => setHeight(e.target.value)} style={{ width: "100%", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: 10, color: C.text }} />
                            </div>
                        </div>

                        <label style={{ display: "block", fontSize: 12, color: C.muted, marginBottom: 4 }}>OBJETIVO PRINCIPAL</label>
                        <select value={goal} onChange={e => setGoal(e.target.value)} style={{ width: "100%", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: 10, color: C.text, marginBottom: 12 }}>
                        ...
                        <select value={philosophy} onChange={e => setPhilosophy(e.target.value)} style={{ width: "100%", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: 10, color: C.text, marginBottom: 12 }}>
                        ...
                        {philosophy === 'classic' && (
                            <>
                                <label style={{ display: "block", fontSize: 12, color: C.muted, marginBottom: 4 }}>ESTILO DE DIVISÃO (CLÁSSICO)</label>
                                <select value={classicStyle} onChange={e => setClassicStyle(e.target.value)} style={{ width: "100%", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: 10, color: C.text, marginBottom: 12 }}>
                        ...
                        <label style={{ display: "block", fontSize: 12, color: C.muted, marginBottom: 6 }}>DIAS DE TREINO NA SEMANA</label>
                        ...
                        <label style={{ display: "block", fontSize: 12, color: C.muted, marginBottom: 4 }}>DURAÇÃO IDEAL (MINUTOS)</label>
                        <select value={duration} onChange={e => setDuration(Number(e.target.value))} style={{ width: "100%", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: 10, color: C.text, marginBottom: 12 }}>
                        ...
                        <label style={{ display: "block", fontSize: 12, color: C.muted, marginBottom: 4 }}>LESÕES / LIMITAÇÕES (Separado por vírgula)</label>
                        <input type="text" placeholder="ex: Joelhos, Lombar, etc" value={injuries} onChange={e => setInjuries(e.target.value)} style={{ width: "100%", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: 10, color: C.text, marginBottom: 20 }} />
                        
                        <button onClick={handleSaveInfo} style={{ width: "100%", background: C.accent, color: C.bg, border: "none", borderRadius: 8, padding: 12, fontFamily: "'Bebas Neue'", fontSize: 18, letterSpacing: 1, cursor: "pointer", marginBottom: 10 }}>
                            CONTINUAR
                        </button>
                    </motion.div>
                )}
                ...
                {step === 'generate' && !plan && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center' }}>
                        <p style={{ fontSize: 13, color: '#fff', marginBottom: 20 }}>
                            A IA vai compilar um plano de {trainingDays.length} dias de treino, adaptado para {equipment}, focando em {duration} mins e salvaguardando o teu perfil biomecânico.
                        </p>
                        <button onClick={handleGenerate} disabled={isGenerating} style={{ width: "100%", background: "linear-gradient(135deg, #e8c84a, #e8a44a)", color: "#000", border: "none", borderRadius: 8, padding: 12, fontFamily: "'Bebas Neue'", fontSize: 18, cursor: isGenerating ? "wait" : "pointer", opacity: isGenerating ? 0.7 : 1 }}>
                            {isGenerating ? "A COMPUTAR MOTOR DE ELITE..." : "✨ GERAR PLANO AGORA"}
                        </button>
                        <button onClick={() => setStep('info')} style={{ width: "100%", background: "none", border: "none", color: C.muted, fontSize: 12, marginTop: 10, cursor: 'pointer', textDecoration: 'underline' }}>Voltar atrás e editar</button>
                    </motion.div>
                )}

                {plan && (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                        <div style={{ background: `${C.accent}22`, border: `1px solid ${C.accent}`, borderRadius: 8, padding: 12, marginBottom: 16 }}>
                            <p style={{ fontSize: 12, color: '#fff', margin: 0, fontStyle: 'italic' }}>"{plan.reasoning}"</p>
                        </div>

                        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                            {plan.workouts.map((day: any, i: number) => {
                                const isRest = day.focus.toLowerCase().includes("descanso") || day.focus.toLowerCase().includes("recupera");
                                return (
                                    <div key={i} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: 12 }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                                            <span style={{ fontFamily: "'Bebas Neue'", fontSize: 16, color: C.accent }}>{day.day}</span>
                                            <span style={{ fontSize: 11, color: C.text, fontWeight: 'bold', background: C.bg, padding: '2px 8px', borderRadius: 4 }}>{day.focus}</span>
                                        </div>
                                        <p style={{ fontSize: 12, color: C.muted, margin: 0, lineHeight: 1.4, marginBottom: isRest ? 0 : 10 }}>
                                            {day.exercises.join(' • ')}
                                        </p>
                                    </div>
                                );
                        ...
                        <button onClick={() => { alert('Plano Ativo no Dashboard!'); onClose(); }} style={{ width: "100%", background: C.accent, color: '#000', border: `none`, borderRadius: 8, padding: 12, fontFamily: "'Bebas Neue'", fontSize: 18, cursor: "pointer", marginTop: 20 }}>
                            APLICAR E FECHAR
                        </button>
                    </motion.div>
                )}
