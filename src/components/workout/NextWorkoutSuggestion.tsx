...
export const NextWorkoutSuggestion = ({ history, profile, onStartWorkout }: Props) => {
  ...
  const suggestion = useMemo(
    () => AIWorkoutGenerator.generateWorkout(recovery, profile, history),
    [recovery, profile, history],
  );
  ...
  if (!suggestion) {
    return (
      ...
      >
        ...
        <h3
          style={{ color: '#eceae4', fontFamily: "'Bebas Neue'", fontSize: 22, letterSpacing: 1 }}
        >
          DIA DE DESCANSO
        </h3>
        <p style={{ color: '#55626e', fontSize: 13, marginTop: 5 }}>
          Os teus músculos estão em recuperação. Faz um cardio leve ou descansa.
        </p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <h3 style={{ color: '#e8c84a', fontSize: 18, marginBottom: 8, marginTop: 10 }}>
        {suggestion.label}
      </h3>
