...
export const PostWorkoutFeedback = ({
  onSubmit,
  profile,
  workoutData,
}: { onSubmit: (feedback: any) => void; profile?: any; workoutData?: any }) => {
  ...
  return (
    ...
    >
      <div style={{ textAlign: 'center', marginBottom: 20 }}>
        <h3
          style={{ color: '#eceae4', fontFamily: "'Bebas Neue'", fontSize: 28, letterSpacing: 1 }}
        >
          TREINO CONCLUÍDO! 🎉
        </h3>
        <p style={{ color: '#e8c84a', fontSize: 16, fontWeight: 'bold' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 5 }}>
          {['easy', 'good', 'hard'].map((level) => (
            <button
              ...
              style={{
                flex: 1,
                background: feedback.difficulty === level ? '#e8c84a22' : '#080b0f',
                border: `1px solid ${feedback.difficulty === level ? '#e8c84a' : '#1e2832'}`,
                borderRadius: 8,
                padding: 8,
                cursor: 'pointer',
                color: '#eceae4',
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <input
          ...
          onChange={(e) => setFeedback({ ...feedback, energy: Number.parseInt(e.target.value) })}
      ...
      <div
        style={{
          background: '#080b0f',
          border: `1px solid ${adaptation.action === 'increase' ? '#3dd68c' : adaptation.action === 'change' ? '#e84a4a' : '#4a9ee8'}`,
          borderRadius: 8,
          padding: 12,
          marginBottom: 20,
        }}
      >
        <p style={{ color: '#eceae4', fontSize: 13, fontWeight: 'bold', marginBottom: 4 }}>
          🤖 Feedback da IA Fittrack:
        </p>
