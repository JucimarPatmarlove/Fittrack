...
export const MuscleRecoveryRing: React.FC<Props> = ({ recoveryData }) => {
    ...
    return (
        <div style={{
            ...
            <p style={{ fontFamily: "'Bebas Neue'", fontSize: 20, letterSpacing: 2, color: C.text, marginBottom: 16 }}>RECUPERAÇÃO MUSCULAR</p>
            ...
            <div style={{ marginTop: 16, textAlign: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
                    <button onClick={() => setCurrentIndex((prev) => prev === 0 ? recoveryData.length - 1 : prev - 1)}
                            style={{ background: 'none', border: 'none', color: C.muted, cursor: 'pointer', fontSize: 18 }}>
                        ◀
                    </button>
                    <p style={{ fontSize: 18, fontWeight: 600, color: current.color, width: 100 }}>{current.muscle}</p>
                    <button onClick={nextMuscle}
                            style={{ background: 'none', border: 'none', color: C.muted, cursor: 'pointer', fontSize: 18 }}>
                        ▶
                    </button>
                </div>
