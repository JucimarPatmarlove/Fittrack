...
export const TrendDashboardSection = ({
  recentExercises,
  maxItems = 4,
}: { recentExercises: string[]; maxItems?: number }) => {
  ...
  return (
    <GlassCard glow style={{ padding: 16, marginBottom: 20 }}>
      ...
      {loading ? (
        <div style={{ fontSize: 12, color: '#55626e', fontFamily: 'monospace' }}>
          Analisando SNC...
        </div>
      ) : (
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                {analysis.suggestedWeightIncrement !== 0 && (
                  <span
                    style={{
                      fontSize: 11,
                      color: analysis.suggestedWeightIncrement > 0 ? '#ccff00' : '#ff6b35',
                      fontFamily: 'monospace',
                    }}
                  >
                    {analysis.suggestedWeightIncrement > 0 ? '+' : ''}
                    {analysis.suggestedWeightIncrement}kg
                  </span>
                )}
