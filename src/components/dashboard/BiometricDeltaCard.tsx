...
import { Scale, Droplets, Activity, Brain } from 'lucide-react';
import { UnifiedHealthMetrics } from '../../services/healthBridge';
...
const MetricBox: React.FC<MetricBoxProps> = ({
  ...
  return (
    <div style={{ background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '12px', padding: '16px', position: 'relative', overflow: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
        <Icon size={16} color="rgba(255,255,255,0.5)" />
        <span style={{ fontSize: '10px', color: 'rgba(255,255,255,0.5)', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '2px' }}>
          {label}
        </span>
      </div>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px' }}>
        <span style={{ fontSize: '1.5rem', fontWeight: 900, fontFamily: 'monospace', color: '#fff', letterSpacing: '-1px' }}>
          {value !== null ? value.toFixed(precision) : '--'}
        </span>
        <span style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.5)', fontWeight: 'bold' }}>{unit}</span>
      </div>
      {trend !== 'neutral' && value !== null && (
        <div style={{ position: 'absolute', top: '16px', right: '16px', color, fontWeight: 'bold', textShadow: `0 0 10px ${color}80` }}>
          {isPositive ? '↑' : '↓'}
        </div>
      )}
    </div>
...
export const BiometricDeltaCard: React.FC<BiometricDeltaCardProps> = ({
  ...
  if (isLoading) {
    return (
      <div style={{ background: 'rgba(255, 255, 255, 0.03)', backdropFilter: 'blur(12px)', borderRadius: '16px', padding: '24px', border: '1px solid rgba(255, 255, 255, 0.1)', width: '100%' }}>
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.75rem', fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: '2px', animation: 'pulse 2s infinite' }}>
  ...
  return (
    ...
    >
      <div style={{ position: 'absolute', top: '-40px', right: '-40px', width: '128px', height: '128px', background: '#00ff88', borderRadius: '50%', filter: 'blur(80px)', opacity: 0.1, pointerEvents: 'none' }} />

      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', position: 'relative', zIndex: 10 }}>
        <h3 style={{ fontFamily: '"Bebas Neue", sans-serif', letterSpacing: '2px', fontSize: '1.25rem', color: '#fff', margin: 0 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '16px', position: 'relative', zIndex: 10 }}>
        <MetricBox
          label="Massa Total"
