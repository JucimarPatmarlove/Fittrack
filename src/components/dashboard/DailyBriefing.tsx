...
import { Calendar, CalendarPlus, CheckCircle, ChevronRight, Dumbbell, Zap } from 'lucide-react';
...
export const DailyBriefing: React.FC<DailyBriefingProps> = ({
  ...
  const yesterdaysWorkout = useMemo(() => {
    ...
    return history.find(
      (w) => w.date === yesterdayString || w.startTime?.startsWith(yesterdayString),
    );
  }, [history]);
    ...
    >
      ...
      >
        <div
          style={{
            ...
            width: '4px',
            height: '100%',
            background: 'linear-gradient(to bottom, #a8e000, transparent)',
            opacity: 0.5,
          }}
        />
        ...
        {yesterdaysWorkout ? (
          <div>
            ...
            <p
              style={{ fontSize: '0.85rem', color: '#9ca3af', margin: 0, fontFamily: 'monospace' }}
            >
              {Math.floor(
                yesterdaysWorkout.totalCalories || yesterdaysWorkout.caloriesBurned || 300,
              )}{' '}
              kcal •{' '}
              <span style={{ color: '#00ff88' }}>
                {yesterdaysWorkout.durationSeconds
                  ? Math.floor(yesterdaysWorkout.durationSeconds / 60)
                  : 45}{' '}
                min
              </span>
            </p>
      ...
      <div
        style={{
          borderRadius: '16px',
          padding: '20px',
          border: `1px solid ${plannedWorkoutToday ? 'rgba(204, 255, 0, 0.3)' : 'rgba(255, 255, 255, 0.1)'}`,
          display: 'flex',
          flexDirection: 'column',
          justifyContent: 'space-between',
          background: plannedWorkoutToday ? 'rgba(204, 255, 0, 0.05)' : 'rgba(255, 255, 255, 0.03)',
          backdropFilter: 'blur(12px)',
        }}
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
          <span style={{ position: 'relative', display: 'flex', width: '12px', height: '12px' }}>
            ...
            <span
              style={{
                ...
                width: '12px',
                height: '12px',
                background: plannedWorkoutToday ? '#ccff00' : '#6b7280',
              }}
            />
          </span>
          <span
        ...
        {plannedWorkoutToday ? (
          ...
          <>
            <div style={{ marginBottom: '16px' }}>
              <h4 style={{ fontSize: '1.1rem', fontWeight: 'bold', color: '#d1d5db', margin: 0 }}>
                Nenhum plano agendado.
              </h4>
            </div>
