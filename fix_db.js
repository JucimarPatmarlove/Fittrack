...
let newContent = content.replace(/export const EXERCISE_DB: Record<string, any> = \{([\s\S]*)\};/m, (match, body) => {
    ...
    if (!newLines.some(l => l.includes('"Running"'))) {
        newLines.push(`  "Running": { muscle: "Cardio", equipment: "Nenhum", type: "cardio", base: { hipertrofia: [] } },`);
        newLines.push(`  "Cycling": { muscle: "Cardio", equipment: "Nenhum", type: "cardio", base: { hipertrofia: [] } },`);
        newLines.push(`  "Rowing": { muscle: "Cardio", equipment: "Máquinas", type: "cardio", base: { hipertrofia: [] } },`);
    }
